...
export default class Myform5 extends Component {
  constructor(props) {
    super(props);
  ...
  clearForm = () => {
    ...
    this.setState({ bio: clean });
  };
  deletebio = (use) => {
    axios.delete(" http://localhost:3201/Bio/" + use.id).then(() => {
      this.getdata();
    });
  };
  getdata = () => {
    axios.get("http://localhost:3201/Bio").then((res) => {
      this.setState({ allBio: res.data });
    });
  };
  editbio = (use, i) => {
  ...
  updatebio = () => {
    axios({
      method: "PUT",
      url: "http://localhost:3201/Bio/" + this.state.bio.id,
      data: this.state.bio,
      headers: {
        "content-Type": "application/json",
      },
    ...
    return (
      <div>
        <form>
          ...
          {/* <button type="button" className="btn btn-primary" onClick={this.addbio} >AddBio</button>
          <button type="button" className="btn btn-secondary" onClick={this.updatebio} >UpdateBio</button> */}
        </form>
        <hr />
  ...
  async componentDidMount() {
    let response = await axios.get("http://localhost:3201/Bio");
    this.setState({ allBio: response.data });
  }
}
