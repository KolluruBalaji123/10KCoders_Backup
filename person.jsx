...
class Person extends Component {
  ...
  getData = () => {
    // let response=await axios.get(" http://localhost:3201/person")
    // this.setState({allInfo:response.data})
    axios.get(" http://localhost:3201/person").then((res) => {
  ...
  addPerson = () => {
    axios({
      method: "POST",
      url: " http://localhost:3201/person/",
  ...
  ClearForm = () => {
    ...
    this.setState({ Info: clean });
  };

  deletePerson = (pers) => {
    axios.delete("http://localhost:3201/person/" + pers.id).then(() => {
      this.getData();
    });
  };
  editPerson = (pers, i) => {
